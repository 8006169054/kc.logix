...
public class PortController {
	...
	public ResponseEntity<PortDto> selectWebsiteTerminalCode(
			@RequestParam(required = false) String hblNo, 
			@RequestParam(required = false) String arrivalNotice
		) throws Exception {
		List<PortDto> PortList = service.selectWebsiteTerminalCode(PortDto.builder().hblNo(hblNo).arrivalNotice(arrivalNotice).build());
...
//				.addData(PortList)
	...
	@GetMapping(value = "/api/basic/website-terminal-code1")
	public ResponseEntity<PortDto> selectWebsiteTerminalCode1(@RequestParam(required = false) String hblNo) throws Exception {
	...
	public ResponseEntity<InputStreamResource> arrivalNoticeSendMail(@RequestBody PortDto paramDto, @KainosSession SessionDto session) throws Exception {
		...
		byte[] eml = null;
		try {
			List<PortDto> portList = service.selectWebsiteTerminalCode(paramDto);
			...
			sb.append("			수신 : " + paramDto.getConcine() + " / 곽혜림 담당자님 <br>                                                      ");
			...
			for (int i = 0; i < portList.size(); i++) {
				PortDto content = portList.get(i);			
				sb.append("			<tr>                                                                                        ");
			...
			mailDto.from(session.getName(), session.getMail()); //보내는사람
			mailDto.addTo("정인선", "is.jung@hmm21.com"); //받는사람
			mailDto.subject("[KCL] Arrival notice 송부의 건, / " + paramDto.getHblNo());
			mailDto.mailbody(sb.toString(), true);
